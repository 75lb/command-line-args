...
class Definitions extends Array {
  ...
  validate () {
    ...
    const numericAlias = this.some(def => {
      invalidOption = def;
      return t$1.isDefined(def.alias) && t$1.isNumber(def.alias)
    });
    if (numericAlias) {
      halt(
        'INVALID_DEFINITIONS',
    ...
    const multiCharacterAlias = this.some(def => {
      invalidOption = def;
      return t$1.isDefined(def.alias) && def.alias.length !== 1
    });
  ...
  whereDefaultValueSet () {
    return this.filter(def => t$1.isDefined(def.defaultValue))
  }
...
function hasDuplicates (array) {
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    if (items[value]) {
      ...
      if (t$1.isDefined(value)) items[value] = true;
    }
  }
}
...
class ArgvParser {
  ...
  constructor (definitions, options) {
    ...
    if (this.argv.hasCombinedShortOptions()) {
      const findReplace = require('find-replace');
      findReplace(this.argv, re.combinedShort, arg => {
        arg = arg.slice(1);
...
class Option {
  ...
  resetToDefault () {
    if (t$1.isDefined(this.definition.defaultValue)) {
...
const camelCase$1 = require('lodash.camelcase');
