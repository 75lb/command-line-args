...
class ArgvIterator {
  ...
  * [Symbol.iterator] () {
    ...
    for (let arg of this.argv) {
      /* match current arg to a option definition */
      if (optionUtil.optEquals.test(arg)) {
        currentDefinition = definitions.get(optionUtil.optEquals.name(arg)) || Definition.create({ name: '_unknown' })
      } else if (optionUtil.isOption(arg)) {
        currentDefinition = definitions.get(arg) || Definition.create({ name: '_unknown' })
      } else {
        currentDefinition = currentDefinition && ((currentDefinition.multiple && currentDefinition.greedy) || !optionValue)
          ? currentDefinition
          : definitions.getDefault() || Definition.create({ name: '_unknown' })
      }
      ...
      /* for stopParsingAtFirstUnknown logic */
      unknownsFound = currentDefinition.name === '_unknown'

      if (unknownsFound && optionUtil.optEquals.test(arg)) {
        yield [ currentDefinition.name, arg, arg ]
      } else {
        yield [ currentDefinition.name, optionValue, arg ]
      }
    }
  }
}
